...
* 配套 SQL 脚本：22 张业务表 DDL、ERP 菜单、字典、角色授权，一键导入；另附一键启动/停止脚本。
...
1. 导入数据库脚本（先建库 `ry-cloud`）：`sql/ry_20260417.sql`、`sql/ry_config_20260611.sql`、`sql/ry_erp_20260818.sql`、`sql/ry_erp_20260819.sql`。
...
ruoyi-ui/                  # 前端（Vue 2 + Element UI）
  src/api/erp/             # ERP mock API（base/purchase/sale/stock/finance/production/report）
...
sql/                       # 数据库脚本（含 ERP 业务表/菜单/字典/报表中心）
