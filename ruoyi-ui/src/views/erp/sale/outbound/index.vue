<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['erp:saleOutbound:add']"
        >新增出库单</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['erp:saleOutbound:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="outboundList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出库单号" align="center" prop="outboundNo" width="160" />
      ...
      <el-table-column label="出库仓库" align="center" prop="warehouseName" :show-overflow-tooltip="true" />
      <el-table-column label="出库日期" align="center" prop="outboundDate" width="120" />
      <el-table-column label="出库金额(元)" align="center" prop="totalAmount" width="130" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="860px" append-to-body>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="客户" prop="customerName">
              <el-select v-model="form.customerName" placeholder="请选择客户" filterable style="width: 100%">
      ...
      <div slot="footer" class="dialog-footer">
        <span class="el-form-item__label" style="margin-right: 16px">出库金额：{{ form.totalAmount }} 元</span>
    ...
    <el-dialog title="销售出库单明细" :visible.sync="openDetail" width="760px" append-to-body>
      <el-descriptions :column="2" border size="small">
        ...
        <el-descriptions-item label="出库金额">{{ detail.totalAmount }} 元</el-descriptions-item>
      </el-descriptions>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listSaleOutbound(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (!this.form.items || this.form.items.length === 0) {
            this.$modal.msgWarning("请添加出库明细行")
            return
          }
          if (this.form.outboundId != undefined) {
            updateSaleOutbound(this.form).then(() => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addSaleOutbound(this.form).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除出库单编号为"' + outboundIds + '"的数据项？').then(function() {
        return delSaleOutbound(outboundIds)
      }).then(() => {
        this.getList()
